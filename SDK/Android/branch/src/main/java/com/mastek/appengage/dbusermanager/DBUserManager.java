...
public class DBUserManager implements Idao {
	...
	public boolean save(/*User user*/) {
		...
		Log.e("Content Values",contentValues.toString());
	...
	public boolean saveCompleteJson(String jsonString) {
		long id = 0;
		dbObject.open();
		final ContentValues contentValues = getContentValues(jsonString);
		Log.e("Content Values",contentValues.toString());
		id = dbObject.db.insertOrThrow(UserTable.TABLE_NAME1, null,
				contentValues);
		Log.d(TAG, "id... : " + id);
		Log.d(TAG, "dbObject... : " + dbObject);
		dbObject.close();
...
//				Log.d(TAG, "Record  : " + user);
	...
	public JSONArray cursorToModel(Cursor cursor) {
		...
		while(cursor.moveToNext())
		{
			...
			try {
				String jsonFromDB = cursor.getString(cursor.getColumnIndex(UserTable.KEY_STRING));
				Log.e("DB1",jsonFromDB);



				object=new JSONObject(jsonFromDB);


				//object.put("key_string", cursor.getString(cursor.getColumnIndex(UserTable.KEY_MANUFACTURER)));
...
//				Log.e("MTTTT",""+cursor.getString(cursor.getColumnIndex(UserTable.KEY_MODEL)));
		...
		for(int i = 0; i <= veryLongString.length() / maxLogSize; i++) {
			...
			Log.e("LongString", veryLongString.substring(start, end));
		}
		return array;
	}

	private ContentValues getContentValues(String jsonString) {
		ContentValues content = new ContentValues();
		content.put(UserTable.KEY_STRING, jsonString);
		return content;
	}
